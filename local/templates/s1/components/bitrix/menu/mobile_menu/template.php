	...
	<div class="mm__menu">
		<nav class="mm__nav">
			<ul class="mm__nav_list">
				...
				?>
					<li class="mm__nav_item<? if ($arItem['ITEM']["SELECTED"]): ?> current<? endif; ?><? if (!empty($sections) || !empty($arItem['CHILDREN'])) : ?> accordion<? endif; ?>">
						<? if (!empty($sections) || !empty($arItem['CHILDREN'])) : ?><span class="accordion_title"><? endif; ?>
							<a href="<?= $arItem['ITEM']["LINK"] ?>" class="link mm__nav_link">
								<?= $arItem['ITEM']["TEXT"] ?>
							</a>
							...
							<? endif; ?>
							<? if (!empty($sections) || !empty($arItem['CHILDREN'])) : ?></span><? endif; ?>
						<? if (!empty($sections)) : ?>
