       ...
       BX.remove(editSteps[i]);
		...
		editActiveBasketBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				basketTable = BX.create('DIV', {props: {className: 'bx-soa-item-table catalog-cart__list'}});
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('DIV', {
								...
								text: parseFloat(BX.message('SOA_SUM_NAME'))
							})
						]
					})
				],
		...
		// 	basketItemsNode.appendChild(
		// 		BX.create('DIV', {
		// 			props: {className: 'bx-soa-item-tr bx-soa-basket-info catalog-cart__item card' + (index == 0 ? ' bx-soa-item-tr-first' : '')},
		// 			children: cols
		// 		})
		// 	);
		...
		createBasketItem: function(basketItemsNode, item, index)
{
    const props = [];

    this.result.GRID.HEADERS.forEach(function(header) {
        ...
        const value = item.data[header.id];
        ...
        props.push(
            BX.create('DIV', {
                ...
                children: [
                    BX.create('SPAN', {
                        props: {
                            className: 'char__item_name'
                        },
                        text: header.name
                    }),
                    BX.create('DIV', {
                        props: {
                            className: 'char__item_value'
                        },
                        text: value
                    })
                ]
    ...
    basketItemsNode.appendChild(
        BX.create('ARTICLE', {
            props: {
                className: 'catalog-cart__item card'
            },
            attrs: {
                'data-id': item.id
            },
            children: [
                ...
                BX.create('DIV', {
                    props: {
                        className: 'catalog-cart__item_image'
                    },
                    children: [
                        BX.create('IMG', {
                            attrs: {
                                src:
                                    item.data.PREVIEW_PICTURE_SRC ||
                                    item.data.DETAIL_PICTURE_SRC ||
                ...
                BX.create('DIV', {
                    props: {
                        className: 'catalog-cart__item_content'
                    },
                    children: [

                        BX.create('DIV', {
                            props: {
                                className: 'catalog-cart__item_col'
                            },
                            children: [
                                BX.create('DIV', {
                                    props: {
                                        className: 'catalog-cart__item_title'
                                    },
                                    children: [
                                        BX.create('H5', {
                                            html: item.data.NAME
                        ...
                        BX.create('DIV', {
                            ...
                            children: [
                                BX.create('DIV', {
                                    props: {
                                        className: 'catalog-cart__item_char'
                                    },
                                    children: [
                                        BX.create('DIV', {
                                            props: {
    ...
    return;
		...
		editActivePropsBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				var propsItemsContainer = this.editPropsItems(propsNode);
				showPropMap && this.editPropsMap(propsNode);

				if (this.params.HIDE_ORDER_DESCRIPTION !== 'Y')
				{
					this.editPropsComment(propsItemsContainer || propsNode);
				}
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'request__form_fields'}}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator;

			if (!propsItemsContainer)
				propsItemsContainer = this.propsBlockNode.querySelector('.request__form_fields');
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			{
				BX.addClass(propsItemNode, "form-group bx-soa-customer-field form__field");
				...
				title = BX.create('DIV', {
					props: {className: 'form__field_title '},
					html: textHtml
				});
				// label = BX.create('LABEL', {
				// 	attrs: {'for': 'soa-property-' + property.getId()},
				// 	props: {className: 'bx-soa-custom-label label-text'},
				// 	html: textHtml
				// });
				propsItemNode.setAttribute('data-property-id-row', property.getId());
				propsItemNode.appendChild(title);
			}
		...
		insertStringProperty: function(property, propsItemNode, disabled)
		{
			...
			{
				propContainer = BX.create('LABEL', {props: {className: 'label-text'}});
				property.appendTo(propContainer);
				propsItemNode.appendChild(propContainer);
				this.alterProperty(property.getSettings(), propContainer);
				this.bindValidation(property.getId(), propContainer);
		...
		alterProperty: function(settings, propContainer)
		{
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				BX.addClass(inputs[i], 'bx-soa-customer-input bx-ios-fix inputtext');
			}
		...
		editPropsComment: function(container)
		{
			var title, input, label, div;
			...
			input = BX.create('INPUT', {
				props: {
					id: 'orderDescription',
					// cols: '4',
					className: ' bx-soa-customer-textarea bx-ios-fix',
					name: 'ORDER_DESCRIPTION'
				},
				text: this.result.ORDER_DESCRIPTION ? this.result.ORDER_DESCRIPTION : ''
			});
			label = BX.create('LABEL', {
				props: {className: 'bx-soa-customer-label label-text'},
				children: [input]
			});
			div = BX.create('DIV', {
