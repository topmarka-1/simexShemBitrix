...
$arProps = [];
$productIds = array_unique(array_column($this->basketItems, 'PRODUCT_ID'));
...
foreach ($this->basketItems as $row) {
	...
	global $USER;
	$rowData['SHOW_PRICE'] = $USER->IsAuthorized() && (float)$row['PRICE'] > 0;
	...
	$mergedColumns = [];
	$skipCode = 'PROPERTY_EDINITSA_IZMERENIYA_OBEMA_VESA_NETTO_VALUE';
	foreach ($rowData['COLUMN_LIST'] as $i => $column) {
		if ($column['CODE'] === 'PROPERTY_OBEM_VES_NETTO_VALUE') {
			foreach ($rowData['COLUMN_LIST'] as $j => $unitColumn) {
				if ($unitColumn['CODE'] === $skipCode) {
					$column['VALUE'] .= ' ' . $unitColumn['VALUE'];
					break;
				}
			}
		}
		if ($column['CODE'] !== $skipCode) {
			$mergedColumns[] = $column;
		}
	}
	$rowData['COLUMN_LIST'] = $mergedColumns;
...
$totalData = array(
	...
	'SHOW_PRICE' => $USER->IsAuthorized() && (float)$result['allSum'] > 0
);
